...
@WebServlet("/insertUsualGrade")
public class InsertUsualGradeServlet extends HttpServlet {
    @Override
...
//        String grade = (String) data.get("grade");
        ...
        // 使用Object获取grade，避免类型不匹配
        Object gradeObjectTemp = data.get("grade");
        String grade = gradeObjectTemp != null ? gradeObjectTemp.toString() : null;

        System.out.println("(insertUsual)grade:" + grade);
        ...
        try {
            ...
            int count = gradeMapper.insertUsualScore(gradeObject);
            ...
            if (count > 0) {
                ...
                sqlSession.rollback();
                resp.getWriter().write("{\"success\": false, \"message\": \"添加成绩失败\"}");
            }
